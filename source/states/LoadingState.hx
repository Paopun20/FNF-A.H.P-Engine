	...
	public static function prepareToSong()
	{
		...
		Thread.create(() -> {
			...
			try
			{
				...
				//function loadfile(typefile) {
				//	try {
				//		var config_song:Dynamic = Song.getConfig(Song.loadedSongName); // GET JSON FILE
				//		if (config_song != null && Reflect.hasField(config_song, 'loadData')) {
				//			if (Reflect.hasField(config_song, typefile)) {
				//				var files = Reflect.field(config_song, typefile); // Extract images field
				//				if (Std.is(files, Array)) { // Check if it's an Array
				//				for (img in (cast files:Array<String>)) {
				//						json.add(typefile+"/" + img);
				//					}
				//				} else {
				//					trace('Error: config_song.'+typefile+' is not an Array.');
				//				}
				//			}
				//		}
				//	} catch (e:Dynamic) {
				//		trace('Error '+typefile+' loading song configuration: ' + e);
				//	}
				//}
				//loadfile('images');
				//loadfile('videos');
