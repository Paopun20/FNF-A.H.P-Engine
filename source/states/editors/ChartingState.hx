...
enum abstract ChartingTheme(String)
{
	...
	var DEFAULT = 'default';
	var VSLICE = 'vslice';
	var ULTRA_LIGHT = 'ultra_light';
	var ULTRA_DARK = 'ultra_dark';
}
...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent
{
	...
	function changeTheme(changeTo:ChartingTheme, ?doSave:Bool = true)
	{
		...
		switch(theme)
		{
			...
			case ULTRA_LIGHT:
        		bg.color = 0xFFFFFFFF;  // Ultra light background (pure white)
        		gridColors = [0xFFE5E5E5, 0xFFD0D0D0]; // Very light grid color
        		gridColorsOther = [0xFFCCCCCC, 0xFFB8B8B8]; // Very light grid color for other elements
			case ULTRA_DARK:
        		bg.color = 0xFF000000;  // Ultra dark background color
        		gridColors = [0xFF1A1A1A, 0xFF121212]; // Very dark grid color
        		gridColorsOther = [0xFF0D0D0D, 0xFF0A0A0A]; // Ultra dark grid color for other elements
			default:
				bg.color = 0xFF303030;
				gridColors = [0xFFDFDFDF, 0xFFBFBFBF];
				gridColorsOther = [0xFF5F5F5F, 0xFF4A4A4A];
	...
	function addSongTab()
	{
		...
		audioOffsetStepper = new PsychUINumericStepper(objX + 180, objY, 1, 0, -1000, 1000, 0);
	...
	function addEditTab()
	{
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, "  Select Down", function() {
			var sel = selectedNotes;
			var selnote:Array<MetaNote> = [];
			for (renderedNote in curRenderedNotes) {
				if (renderedNote.y >= vortexIndicator.y) {
					selnote.push(renderedNote);
				}
			}
			selectedNotes = selnote.copy();
			trace("Vortex Indicator Position: " + vortexIndicator.y);
			addUndoAction(SELECT_NOTE, { old: sel, current: selnote.copy() });
			onSelectNote();
			trace("Notes selected: " + selectedNotes.length);
		}, btnWid);
		btn.text.alignment = LEFT;
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Select Up', function()
		{
			var sel = selectedNotes;
			var selnote:Array<MetaNote> = [];
			for (renderedNote in curRenderedNotes) {
			...
			function(state:BasePrompt)
				{
					...
					var btn:PsychUIButton = new PsychUIButton(0, btnY, 'Ultra Light', changeTheme.bind(ULTRA_LIGHT));
					btn.screenCenter(X);
					btn.x += 240;
					btn.cameras = state.cameras;
					state.add(btn);
				
					var btn:PsychUIButton = new PsychUIButton(0, btnY, 'Ultra Dark', changeTheme.bind(ULTRA_DARK));
					btn.screenCenter(X);
					btn.x += 360;
					btn.cameras = state.cameras;
					state.add(btn);
